...
export const EditWineType = ({ wineType, onSave, onCancel }: EditWineProps) => {
  const { updateToast } = useToast();
  const { updateAppLoading } = useAppState();
  const { wineTypes } = useRealTimeDb();
  const [value, setValue] = useState<string>(wineType);
  ...
  const handleUpdateWineTypes = async () => {
    ...
    const newWineTypes = wineTypes.map((wine) => {
      if (wine === wineType) {
        return value;
      }
      return wine;
    ...
    <div className="flex items-center justify-center w-full h-full fixed top-0 left-0 bg-surface/80 backdrop-blur-sm">
      ...
      >
        <Container intent="flexColLeft" gap="medium">
          <Text intent="h3">Edit Wine Type</Text>
        </Container>
